...
/*!
    ...
    Written by Kevin "KTOWN" Townsend for Adafruit Industries and Samuel Renovell González.
...
int16_t Adafruit_ADS1X15::getADC_SingleEnded(uint8_t channel) {
  if (channel > 3) {
    return 0;
  }
...
void Adafruit_ADS1X15::askADC_SingleEnded(uint8_t channel) {
  if (channel > 3) {
    return;
  }
...
int16_t Adafruit_ADS1X15::readADC() {
...
int16_t Adafruit_ADS1X15::readAndNextADC_SingleEnded(uint8_t channel) {
  if (channel > 3) {
    return 0;
  }
...
int16_t Adafruit_ADS1X15::read_and_ask_ADC_Differential_0_1() {
  // Start with default values
  uint16_t config =
      ADS1X15_REG_CONFIG_CQUE_NONE |    // Disable the comparator (default val)
      ADS1X15_REG_CONFIG_CLAT_NONLAT |  // Non-latching (default val)
...
int16_t Adafruit_ADS1X15::read_and_ask_ADC_Differential_2_3() {
  // Start with default values
  uint16_t config =
      ADS1X15_REG_CONFIG_CQUE_NONE |    // Disable the comparator (default val)
      ADS1X15_REG_CONFIG_CLAT_NONLAT |  // Non-latching (default val)
  ...
  // Set channels
  config |= ADS1X15_REG_CONFIG_MUX_DIFF_2_3; // AIN0 = P, AIN1 = N
